...
# This class uses for Train the Chatbot 
class BotTrainer(object):
    ...
    def train(self, result_dir, target=""):
        ...
        config_proto = tf.ConfigProto(log_device_placement=log_device_placement, allow_soft_placement=True)
        ...
        with tf.Session(target=target, config=config_proto, graph=self.graph) as sess:
            ...
            print(colorama.Fore.GREEN + '\n\nTraining started ...{} '+format(time.strftime("%Y-%m-%d %H:%M:%S")))           
            print(f'Total Epochs  : {num_epochs}')
            print(f'Current Epochs: {train_epoch}')
            ...
            while train_epoch <= num_epochs:
                ...
                print(colorama.Fore.YELLOW+f'Training epoch {train_epoch}'+colorama.Fore.RESET)
                ...
                try:
                    ...
                    print(f'ckpt_predict_count : {ckpt_predict_count}')
...
if __name__ == "__main__":
    ...
    "Must be run in Python 3.5 or later. You are running {}".format(sys.version)
